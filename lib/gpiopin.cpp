...
void CGPIOPin::SetMode (TGPIOMode Mode, boolean bInitPin)
{
	...
	assert (m_nPin < GPIO_PINS);
	u64 nSelReg = ARM_GPIO_GPFSEL0 + (m_nPin / 10) * 4;
...
void CGPIOPin::Write (unsigned nValue)
{
	...
	assert (m_nPin < GPIO_PINS);
	u64 nSetClrReg = (m_nValue ? ARM_GPIO_GPSET0 : ARM_GPIO_GPCLR0) + (m_nPin / 32) * 4;
	u32 nShift = m_nPin % 32;
...
unsigned CGPIOPin::Read (void) const
{
	...
	assert (m_nPin < GPIO_PINS);
	u64 nLevReg = ARM_GPIO_GPLEV0 + (m_nPin / 32) * 4;
	u32 nShift = m_nPin % 32;
...
void CGPIOPin::EnableInterrupt (TGPIOInterrupt Interrupt)
{
	...
	assert (m_nPin < GPIO_PINS);
	u64 nReg =   ARM_GPIO_GPREN0
		   + (m_nPin / 32) * 4
		   + (Interrupt - GPIOInterruptOnRisingEdge) * 12;

	u32 nMask = 1 << (m_nPin % 32);
...
{
	assert (m_nPin < GPIO_PINS);
	u64 nClkReg = ARM_GPIO_GPPUDCLK0 + (m_nPin / 32) * 4;
	u32 nShift = m_nPin % 32;
...
void CGPIOPin::DisableAllInterrupts (unsigned nPin)
{
	...
	u64 nReg = ARM_GPIO_GPREN0 + (nPin / 32) * 4;
	u32 nMask = 1 << (nPin % 32);
