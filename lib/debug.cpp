...
static CGPIOPin AudioLeft (41, GPIOModeOutput);
static CGPIOPin AudioRight (40, GPIOModeOutput);
...
void debug_hexdump (const void *pStart, unsigned nBytes, const char *pSource)
{
	...
	CLogger::Get ()->Write (pSource, LogDebug, "Dumping 0x%X bytes starting at 0x%lX", nBytes, (unsigned long) pOffset);
...
void debug_stacktrace (const u64 *pStackPtr, const char *pSource)
{
	if (pSource == 0)
	{
		pSource = FromDebug;
